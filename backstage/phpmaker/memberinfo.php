...
class cmember extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('member', 'member', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// userid
		$this->_userid = new cField('member', 'member', 'x__userid', 'userid', '`userid`', '`userid`', 3, -1, FALSE, '`userid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->_userid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userid'] = &$this->_userid;

		// username
		$this->username = new cField('member', 'member', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['username'] = &$this->username;

		// idtype
		$this->idtype = new cField('member', 'member', 'x_idtype', 'idtype', '`idtype`', '`idtype`', 3, -1, FALSE, '`idtype`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idtype->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idtype'] = &$this->idtype;

		// MasPass
		$this->MasPass = new cField('member', 'member', 'x_MasPass', 'MasPass', '`MasPass`', '`MasPass`', 200, -1, FALSE, '`MasPass`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['MasPass'] = &$this->MasPass;

		// accessToken
		$this->accessToken = new cField('member', 'member', 'x_accessToken', 'accessToken', '`accessToken`', '`accessToken`', 200, -1, FALSE, '`accessToken`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['accessToken'] = &$this->accessToken;
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->MasPass);
				$Doc->ExportCaption($this->accessToken);
			} else {
				$Doc->ExportCaption($this->id);
				$Doc->ExportCaption($this->_userid);
				$Doc->ExportCaption($this->username);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->MasPass);
					$Doc->ExportField($this->accessToken);
				} else {
					$Doc->ExportField($this->id);
					$Doc->ExportField($this->_userid);
					$Doc->ExportField($this->username);
