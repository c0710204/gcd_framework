...
//
// Table class for activitytime
//
class cactivitytime extends cTable {
	var $id;
	var $startime;
	var $endtime;
	var $addressId;
	...
	function __construct() {
		global $Language;
		$this->TableVar = 'activitytime';
		$this->TableName = 'activitytime';
		...
		// id
		$this->id = new cField('activitytime', 'activitytime', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// startime
		$this->startime = new cField('activitytime', 'activitytime', 'x_startime', 'startime', '`startime`', 'DATE_FORMAT(`startime`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`startime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->startime->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['startime'] = &$this->startime;

		// endtime
		$this->endtime = new cField('activitytime', 'activitytime', 'x_endtime', 'endtime', '`endtime`', 'DATE_FORMAT(`endtime`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`endtime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->endtime->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['endtime'] = &$this->endtime;

		// addressId
		$this->addressId = new cField('activitytime', 'activitytime', 'x_addressId', 'addressId', '`addressId`', '`addressId`', 3, -1, FALSE, '`EV__addressId`', TRUE, FALSE, TRUE, 'FORMATTED TEXT');
		$this->addressId->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['addressId'] = &$this->addressId;

		// address
		$this->address = new cField('activitytime', 'activitytime', 'x_address', 'address', '`address`', '`address`', 200, -1, FALSE, '`address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address'] = &$this->address;

		// activityid
		$this->activityid = new cField('activitytime', 'activitytime', 'x_activityid', 'activityid', '`activityid`', '`activityid`', 3, -1, FALSE, '`activityid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->activityid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['activityid'] = &$this->activityid;
	}
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT `roomName` FROM `classroom` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `activitytime`.`addressId` LIMIT 1) AS `EV__addressId` FROM `activitytime`" .
			") `EW_TMP_TABLE`";
	}
	...
	function UseVirtualFields() {
		...
		if ($this->addressId->AdvancedSearch->SearchValue <> "" ||
			$this->addressId->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->addressId->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->addressId->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("activitytimeview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "activitytimeadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("activitytimeedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("activitytimeadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("activitytimedelete.php", $this->UrlParm());
	}
