...
class controller
{
	...
	function _loadloopview_html_ng($html,$data)
	{
		...
		if ($this->echo) echo $output; else return $output;	
		return 0;
	}*/
	...
	function loadloopview($uri,$datas)
	{
		...
		{
			$htmldefult=file_get_contents($_SERVER['DOCUMENT_ROOT'].__CFG_document_place__."/includes/view/$uri");
			if ($this->echo)echo  $this->_loadloopview_html($htmldefult, $datas); else return $this->_loadloopview_html($htmldefult, $datas);
